...
def add_point_to_graph(new_xy, adjacency_dict, building_mask, image_to_draw, ignore_building=False):
    """
    מוסיפה את הנקודה new_xy = (x, y) כצומת חדשה ב-adjacency_dict,
    ומקשרת אותה לצומת הקיימת הקרובה ביותר שהקו אליה לא חוצה מבנה, אלא אם כן
    ignore_building הוא True, ואז מתעלמים מבדיקת מעבר הקו.
    
    אם לא נמצא אף צומת מתאים – מחזירה None.
    אחרת, מחזירה new_xy.
    
    בנוסף, מסמנת את הנקודה (בצבע צהוב) ואת הקו (באדום) בתמונה image_to_draw.
    """
    (new_x, new_y) = new_xy

    existing_nodes = list(adjacency_dict.keys())
    if not existing_nodes:
        return None  # אין צמתים להתחבר אליהם
    ...
    for candidate_node in existing_nodes_sorted:
        ...
        # אם ignore_building הוא True, מתעלמים מבדיקה זו.
        if ignore_building or not line_intersects_building(new_x, new_y, x2, y2, building_mask):
            ...
            # סימון על התמונה: צומת חדשה בצהוב וקו אדום לצומת
            dep.cv2.circle(image_to_draw, new_xy, 3, (0, 255, 255), -1)  # צהוב ב-BGR
            dep.cv2.line(image_to_draw, new_xy, candidate_node, (0, 0, 255), 2)  # אדום ב-BGR
