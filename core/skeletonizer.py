...
def merge_images(binary_image, refined_skeleton):
    """
    פונקציה שממזגת את הבינארי (מבנים) ואת השלד (refined_skeleton) לצורך הדמיה.
    buildings = לבן, skeleton = כחול.
    """
    h, w = binary_image.shape
    merged_image = dep.np.zeros((h, w, 3), dtype=dep.np.uint8)

    # המבנים (בינארי==1) בצבע לבן
    merged_image[binary_image == 1] = [255, 255, 255]

    # השלד (refined_skeleton==1) בצבע כחול
    merged_image[refined_skeleton == 1] = [255, 0, 0]  # או [0,0,255] תלוי BGR או RGB
